...
?>
<?php
	echo "\t<?php if(\$show_approvals) { ?>\n";
	echo "\t\t<div class=\"clearfix\"></div><div class=\"panel\"> <div class=\"panel-heading\"><h3 class=\"panel-title\"><?php echo __(\"Send for approval\") ?></h3></div><div class=\"panel-body\"><?php echo __(\"Records added to this table will be send to the person you choose from the list below.\")?>\n";
	echo "\t\t\t<?php echo \$this->Form->input('Approval.employee_id',array('options'=>\$employee_id));?>\n";
	echo "\t\t\t<?php echo \$this->Form->input('Approval.comments',array('type'=>'textarea'));?>\n";
	echo "\t\t<?php if(\$same == \$this->Session->read('User.employee_id'))echo \$this->Form->input('publish',array('label'=>'Do not send forward. Publish Now')) ?>\n";
	echo "\t</div>\n";
	?>
		<h4>&nbsp;&nbsp;&nbsp;<?php echo __('History') ?></h4>
		<table class="table">
				<tr><th>Date</th><th>From</th><th>Comments</th><th>Sent to</th></tr>
		<?php echo "<?php foreach(\$approval_history as \$history): ?>\n";?>
				<tr>
						<td><?php echo "<?php echo \$history['Approval']['created'] ?>" ?></td>
						<td><?php echo "<?php echo \$history['Approval']['from'] ?>" ?></td>
						<td><?php echo "<?php echo \$history['Approval']['comments'] ?>" ?></td>
						<td><?php echo "<?php echo \$history['Approval']['employee_id'] ?>" ?></td>
				</tr>
...
<div class="col-md-4">
	<fieldset><legend>More...</legend></fieldset>
